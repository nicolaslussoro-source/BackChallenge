...
export async function getUser(userId: string): Promise<{ id: string; email: string; name: string; created_at: string | null; last_login: string | null; login_count: number } | null> {
    const rows = await pool.query('SELECT id, name, email, created_at, last_login, login_count FROM users WHERE id = ?', [userId])
